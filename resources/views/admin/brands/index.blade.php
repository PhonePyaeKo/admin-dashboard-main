                ...
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <div class="text-base font-semibold text-gray-900">
                            {{ __('labels.brand.title') }}
                        </div>
                        <p class="mt-2 text-sm text-gray-700">{{ __('labels.brand.description') }}</p>
                    </div>
                    @can('brand_create')
                        <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                            <a href="{{ route('admin.brands.create') }}"
                                ...
                                {{__('global.add')}} {{__('labels.brand.title_singular')}}
                            </a>
                        </div>
                    @endcan
                </div>
                            ...
                            @foreach ($brands as $brand)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 flex items-center justify-end gap-2">
                                        ...
                                        @endcan
                                        @can('brand_delete')
                                            <x-admin.delete-popup :id="$brand->id" :action="route('admin.brands.destroy', $brand->id)" :isDestroy="true">
                                                <button type="submit"
